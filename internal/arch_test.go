...
func internalPackages(t *testing.T) []string {
	...
	pkgs, err := packages.Load(cfg, "github.com/seamless-ssh/sssh/internal/...")
	if err != nil {
		t.Fatalf("failed to load internal packages: %v", err)
	}
	const domain = "github.com/seamless-ssh/sssh/internal/domain"
	const root = "github.com/seamless-ssh/sssh/internal"
	var result []string
...
func TestArchitecture_Dependencies(t *testing.T) {
	const modulePrefix = "github.com/seamless-ssh/sssh/"
...
func TestArchitecture_NoGlobals(t *testing.T) {
	for _, pkgPath := range internalPackages(t) {
		if pkgPath == "github.com/seamless-ssh/sssh/internal/fs" {
			continue
		}
		t.Run(pkgPath, func(t *testing.T) {
